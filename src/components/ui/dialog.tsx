...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Overlay
    ref={ref}
    className={cn(
      'fixed inset-0 z-50 bg-void/75 backdrop-blur-md',
...
>(({ className, children, hideClose, ...props }, ref) => (
  <DialogPrimitive.Portal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        'glass-solid fixed left-1/2 top-1/2 z-50 grid w-[calc(100vw-2rem)] max-w-lg -translate-x-1/2 -translate-y-1/2 gap-5 rounded-xl p-7 shadow-lift duration-300',
    ...
    >
      <div
        aria-hidden
        className="pointer-events-none absolute inset-x-10 -top-px h-px bg-linear-to-r from-transparent via-brand-400/70 to-transparent"
      />
      {children}
      {!hideClose ? (
